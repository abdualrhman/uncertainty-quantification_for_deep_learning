...
def experiment(models: list, train_set, test_set, num_trials: int, bsz: int, num_train_epoch: int, train_bool):
    top1s = np.zeros((num_trials,))
    f1s = np.zeros((num_trials,))
    for i in tqdm(range(num_trials)):
        top1_avg, f1score_avg = trial(
            models,  train_set, test_set, bsz, num_train_epoch, train_bool)
        top1s[i] = top1_avg
        f1s[i] = f1score_avg
        print(
...
def trial(modelnames: list, train_set, test_set, bsz: int, num_train_epoch: int, train_bool: bool = True):
