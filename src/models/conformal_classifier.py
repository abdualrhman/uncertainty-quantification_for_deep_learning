...
from src.utils.cp_utils import validate, get_logits_targets, sort_sum


class ConformalClassifier(nn.Module):
...
class ConformalClassifierLogits(nn.Module):
    def __init__(self, model, calib_loader, alpha, kreg=None, lamda=None, randomized=True, allow_zero_sets=False, naive=False, LAC=False, pct_paramtune=0.3, batch_size=32, lamda_criterion='size', device='cpu'):
        super(ConformalClassifierLogits, self).__init__()
        self.model = model
        self.alpha = alpha
        self.randomized = randomized
        self.LAC = LAC
...
def platt_logits(cmodel, calib_loader, max_iters=10, lr=0.01, epsilon=0.01):
    nll_criterion = nn.CrossEntropyLoss().to("cpu")

    T = nn.Parameter(torch.Tensor([1.3]).to('cpu'))
    ...
    for iter in range(max_iters):
        ...
        for x, targets in calib_loader:
            ...
            out = x/T
            loss = nll_criterion(out, targets.long().to('cpu'))
            loss.backward()
            optimizer.step()
...
def pick_lamda_size(model, paramtune_loader, alpha, kreg, randomized, allow_zero_sets):
    ...
    for temp_lam in [0.001, 0.01, 0.1, 0.2, 0.5]:
        conformal_model = ConformalClassifierLogits(model, paramtune_loader, alpha=alpha, kreg=kreg,
                                                    lamda=temp_lam, randomized=randomized, allow_zero_sets=allow_zero_sets, naive=False)
        top1_avg, top5_avg, f1score, cvg_avg, sz_avg = validate(
